...
<!-- endinject -->
    ...
    <div class="container-scroller">
      ...
      <nav class="navbar default-layout-navbar col-lg-12 col-12 p-0 fixed-top d-flex flex-row">
        <div class="text-center navbar-brand-wrapper d-flex align-items-center justify-content-center">
          <a class="navbar-brand brand-logo" href="/dashboard"><img src="{{ asset('assets/images/fm1remove2.png') }}" style="width: 150px; height: 50px;" alt="logo" />
</a>
  ...
  <li class="nav-item">
    ...
    <div class="col-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Modifier une Taches</h4>
                    <p class="card-description"> Cordonnées Du Tache </p>
                    <form class="forms-sample" action="{{ route('tache.update', ['id' => $tache->id]) }}" method="POST">
  @csrf
  <div class="form-group row">
    <div class="col-md-8">
      <label for="name">Désignation</label>
      <input type="text" name="designation" id="name" class="form-control" required autofocus value="{{ $tache->designation }}">
    </div>
    <div class="col-md-4">
      <label for="role">Employées</label>
      <select name="userid" id="role" class="form-control" required>
        <option value="" disabled>1er Employé (Obligatoire)</option>
        @foreach($users as $user)
        <option value="{{ $user->id }}" {{ $tache->userid == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
        @endforeach
      </select>

      <select name="userid2" id="role" class="form-control" required>
        <option value="" disabled>2eme Employé (Facultatif)</option>
        @foreach($users as $user)
        <option value="{{ $user->id }}" {{ $tache->userid2 == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
        @endforeach
      </select>

      <select name="userid3" id="role" class="form-control" required>
        <option value="" disabled>3eme Employé (Facultatif)</option>
        @foreach($users as $user)
        <option value="{{ $user->id }}" {{ $tache->userid3 == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
        @endforeach
      </select>
  ...
  <div class="form-group row">
    <div class="col-md-8">
      <label for="name">Commentaire (facultatif)</label>
      <input type="text" name="commentaire" id="name" class="form-control" autofocus value="{{ $tache->commentaire }}">
    </div>

    <div class="col-md-4">
      <label for="numclient">Clients</label>
                                      <!-- search  -->
                                      <input type="text" id="searchClient" class="form-control search-input" placeholder="Chercher">
                                <!-- fin search  -->
      <select name="numclient" id="clientselect" class="form-control" required>
        <option value="" disabled>Choisissez un Client</option>
        @foreach($clients as $client)
        <option value="{{ $client->numclient }}" {{ $tache->numclient == $client->numclient ? 'selected' : '' }}>{{ $client->nameclient }}</option>
        @endforeach
      </select>
    </div>
  </div>
  ...
  <div class="form-group row">
    <div class="col-md-6">
      <label for="exampleInputName1">Date de Tache</label>
      <input type="datetime-local" class="form-control" id="exampleInputName1" name="datetache" required value="{{ $tache->datetache }}">
    </div>

    <div class="col-md-3">
      <label for="exampleInputName1">Priorité</label>
      <select name="priorité" id="role" class="form-control" required>
    <option value="Basse" {{ $tache->priorité == 'Basse' ? 'selected' : '' }}>Basse</option>
    <option value="Moyenne" {{ $tache->priorité == 'Moyenne' ? 'selected' : '' }}>Moyenne</option>
    <option value="Elevé" {{ $tache->priorité == 'Elevé' ? 'selected' : '' }}>Élevée</option>
</select>
    ...
    <div class="col-md-3">
      ...
      <select name="etat" id="role" class="form-control" required>
    <option value="ouverte" {{ $tache->etat == 'ouverte' ? 'selected' : '' }}>ouverte</option>
        <option value="annulée" {{ $tache->etat == 'en cours' ? 'selected' : '' }}>en cours</option>
  ...
  <a href="/tache" class="btn btn-primary mr-2">Fermer</a>
  <button type="submit" class="btn btn-success mr-2">Mettre à jour la Tâche</button>
</form>
