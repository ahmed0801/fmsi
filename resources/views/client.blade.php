...
@endif  
  ...
  <input type="text" class="form-control" placeholder="Rechercher par numéro ou nom de client" id="searchInput">
  <div class="input-group-append">
    <button class="btn btn-outline-secondary" type="button">
      <i class="mdi mdi-magnify"></i>
                    ...
                    <!-- fin teste filtre  -->
                    <p class="card-description"> Toutes Les Clients FMSI
                    </p>
                    ...
                    <table class="table table-striped" id="clientTable">
                      ...
                      @foreach ($clients as $client)
                        ...
                        <td> {{ $client->numclient }} <br> {{ $client->nameclient }} </td>
                          
                        @if($client->type == "particulier")
                        <td> Particulier / Syndic </td>
                        @else 
                        <td> {{ $client->type }} </td>
                        @endif
                          <td> {{ $client->tel1 }} <br> {{ $client->tel2 }} </td>
                          <td> {{ $client->mail }} </td>
                          <!-- juste pour l'alignement -->
                          <td class="adresse-cell">
                             <?php
                             $adresse = $client->adresse;
                          $adresse = wordwrap($adresse, 20, "<br>", true); // Remplacez 20 par le nombre de caractères maximum par ligne
                         echo $adresse;
                          ?>
                          </td>
