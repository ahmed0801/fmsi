...
@endif
  ...
  <select class="form-control ml-2" id="statusInput">
    ...
    <option value="annulée">Annulée</option>
                    ...
                    <p class="card-description"> Attention : Chaque Filtre Est Indépendant Des Autres Filtres
                    </p>
                    ...
                    <table class="table table-striped" id="clientTable">
                      ...
                      @foreach ($taches as $tache)
                        ...
                        <?php
                          ...
                          $designation = wordwrap($designation, 20, "<br>", true); // Remplacez 20 par le nombre de caractères maximum par ligne
                         echo $designation;
                          ?>
                        </td>
                          <!-- <td> {{ $tache->commentaire }} </td> -->
                          <td> {{ $tache->username }} </td>
                          <td> {{ $tache->numclient }} </td>
                          <td> {{ $tache->nameclient }} </td>
                          <td> 
                            @if($tache->etat == "ouverte")
                            <span class="badge badge-warning">{{ $tache->etat }}</span>
                            @elseif($tache->etat == "terminée")
                            <span class="badge badge-success">{{ $tache->etat }}</span>
                            @else 
                            <span class="badge badge-danger">{{ $tache->etat }}</span>
                            @endif
                            </td>
                          <td> {{ $tache->datetache }} </td>
                          <td> 
                          <a href="{{ route('edittache', ['id' => $tache->id]) }}"> <!-- Lien vers la page d'édition de la tâche -->
